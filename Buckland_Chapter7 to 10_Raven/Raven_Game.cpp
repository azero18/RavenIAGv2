...
//-----------------------------------------------------------------------------
void Raven_Game::AddBots(unsigned int NumBotsToAdd, unsigned int Type)
{ 
  if (Type == 0)//NEW: adicionar bot padrao
  {
	  while (NumBotsToAdd--)
	  {
		//create a bot. (its position is irrelevant at this point because it will
		//not be rendered until it is spawned)
		Raven_Bot* rb = new Raven_Bot_X(this, Vector2D()); //NEW adiciona Raven_Bot_X (original era Raven_Bot)
	...
	#ifdef LOG_CREATIONAL_STUFF
	  debug_con << "Adding bot type 1 with ID " << ttos(rb->ID()) << "";
	#endif
	  }
  }
  if (Type == 1)//NEW: adicionar bot modificado
  {
	  while (NumBotsToAdd--)
	  {
		//create a bot. (its position is irrelevant at this point because it will
		//not be rendered until it is spawned)
		Raven_Bot* rb = new Raven_Bot_Z(this, Vector2D());
	...
	#ifdef LOG_CREATIONAL_STUFF
	  debug_con << "Adding bot type 2 with ID " << ttos(rb->ID()) << "";
	#endif
	  }
  }
}
...
{  
  ...
  { 
	//NEW: Iniciar com 2 tipos de bots
    AddBots(script->GetInt("NumBots0"),0);
