#ifndef GOAL_COMPOSITEZ_H
#define GOAL_COMPOSITEZ_H
...
class Goal_Composite_Z : public Goal_Z<entity_type>
{
private:

    typedef std::list<Goal_Z<entity_type>* > SubgoalList;
...
public:
  ...
  Goal_Composite_Z(entity_type* pE, int type):Goal_Z<entity_type>(pE,type){}

  //when this object is destroyed make sure any subgoals are terminated
  //and destroyed.
  virtual ~Goal_Composite_Z(){RemoveAllSubgoals();}
  ...
  void         AddSubgoal(Goal_Z<entity_type>* g);
...
//-----------------------------------------------------------------------------
template <class entity_type>
void Goal_Composite_Z<entity_type>::RemoveAllSubgoals()
{
...
//-----------------------------------------------------------------------------
template <class entity_type>
int Goal_Composite_Z<entity_type>::ProcessSubgoals()
{ 
...
//----------------------------- AddSubgoal ------------------------------------
template <class entity_type>
void Goal_Composite_Z<entity_type>::AddSubgoal(Goal_Z<entity_type>* g)
{   
...
//-----------------------------------------------------------------------------
template <class entity_type>
bool Goal_Composite_Z<entity_type>::ForwardMessageToFrontMostSubgoal(const Telegram& msg)
{
  if (!m_SubGoals.empty())
  {
...
//-------------------------- RenderAtPos --------------------------------------
template <class entity_type>
void  Goal_Composite_Z<entity_type>::RenderAtPos(Vector2D& pos, TypeToString* tts)const
{
  Goal_Z<entity_type>::RenderAtPos(pos, tts);
...
template <class entity_type>
void  Goal_Composite_Z<entity_type>::Render()
{
  if (!m_SubGoals.empty())
  {
