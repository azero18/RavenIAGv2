...
//-------------------------- ctor ---------------------------------------------
Raven_Bot_Original::Raven_Bot_Original(Raven_Game* world,Vector2D pos):
  ...
  Raven_Bot(world, pos),
                 ...
                 m_bHit(false),
                 m_iScore(0),
				 m_iDeaths(0), //NEW: resetar para 0 o contador de mortes
				 m_dSurvivedTime(0), //NEW: resetar para 0 o contador de tempo de sobrevivencia
                 m_Status(spawning),
                 m_bPossessed(false),
...
//-------------------------------- dtor ---------------------------------------
//-----------------------------------------------------------------------------
Raven_Bot_Original::~Raven_Bot_Original() //NEW: imprimindo as estatisticas antes de ser deletado
{
  ...
  debug_con << "deleting raven bot (id = " << ID() << ")" << "\n"
	  << "this a original bot.\nSTATISTICS:" << typeid(this).name()
		    << "\n - Kills = " << Score() 
			<< "\n - Deaths = " << Deaths()
			<< "\n - Time has survived = " << TimeSurvive() << "s (mean = " << TimeSurvive()/(Deaths()+1) << "s/lives)\n\n";
...
//-----------------------------------------------------------------------------
void Raven_Bot_Original::Spawn(Vector2D pos)
{
...
//-------------------------------- Update -------------------------------------
//
void Raven_Bot_Original::Update()
{
...
//-----------------------------------------------------------------------------
void Raven_Bot_Original::UpdateMovement()
{
...
//-----------------------------------------------------------------------------
bool Raven_Bot_Original::isReadyForTriggerUpdate()const
{
...
//--------------------------- HandleMessage -----------------------------------
//-----------------------------------------------------------------------------
bool Raven_Bot_Original::HandleMessage(const Telegram& msg)
{
...
//----------------------------------------------------------------------------
bool Raven_Bot_Original::RotateFacingTowardPosition(Vector2D target)
{
...
//--------------------------------- ReduceHealth ----------------------------
void Raven_Bot_Original::ReduceHealth(unsigned int val)
{
  m_iHealth -= val;

  if (m_iHealth <= 0) //NEW: adicionado a condiccao de 'isAlive()'
  {
	SetTimeOfDeath(); //NEW: marque a hora de sua morte
	IncrementDeaths(); //NEW: aumente seu contador de mortes
	IncrementSurviveTime(); //NEW: aumente seu contador de tempo de vida
    SetDead();
  }
...
//-----------------------------------------------------------------------------
void Raven_Bot_Original::TakePossession()
{
...
//-----------------------------------------------------------------------------
void Raven_Bot_Original::Exorcise()
{
  m_bPossessed = false;
...
//----------------------- ChangeWeapon ----------------------------------------
void Raven_Bot_Original::ChangeWeapon(unsigned int type)
{
...
//-----------------------------------------------------------------------------
void Raven_Bot_Original::FireWeapon(Vector2D pos)
{
...
//-----------------------------------------------------------------------------
double Raven_Bot_Original::CalculateTimeToReachPosition(Vector2D pos)const
{
...
//-----------------------------------------------------------------------------
bool Raven_Bot_Original::isAtPosition(Vector2D pos)const
{
...
//-----------------------------------------------------------------------------
bool Raven_Bot_Original::hasLOSto(Vector2D pos)const
{
...
bool Raven_Bot_Original::canWalkTo(Vector2D pos)const
{
...
bool Raven_Bot_Original::canWalkBetween(Vector2D from, Vector2D to)const
{
...
//-----------------------------------------------------------------------------
bool Raven_Bot_Original::canStepLeft(Vector2D& PositionOfStep)const
{
...
bool Raven_Bot_Original::canStepRight(Vector2D& PositionOfStep)const
{
  static const double StepDistance = BRadius() * 2;
...
bool Raven_Bot_Original::canStepForward(Vector2D& PositionOfStep)const
{
  static const double StepDistance = BRadius() * 2;
...
bool Raven_Bot_Original::canStepBackward(Vector2D& PositionOfStep)const
{
  static const double StepDistance = BRadius() * 2;
...
//
//------------------------------------------------------------------------
void Raven_Bot_Original::Render()                                         
{
...
//------------------------- SetUpVertexBuffer ---------------------------------
//-----------------------------------------------------------------------------
void Raven_Bot_Original::SetUpVertexBuffer()
{
...
void Raven_Bot_Original::RestoreHealthToMaximum(){m_iHealth = m_iMaxHealth;}

void Raven_Bot_Original::IncreaseHealth(unsigned int val)
{
  m_iHealth+=val; 
