...
//-----------------------------------------------------------------------------
Goal_FollowPath_Z::
Goal_FollowPath_Z(Raven_Bot_Z*          pBot,
                std::list<PathEdge> path):Goal_Composite_Z<Raven_Bot_Z>(pBot, goal_follow_path_Z),
                                                  m_Path(path)
{
...
void Goal_FollowPath_Z::Activate()
{
  ...
  case NavGraphEdge::normal:
    {
      AddSubgoal(new Goal_TraverseEdge_Z(m_pOwner, edge, m_Path.empty()));
    }
  ...
  case NavGraphEdge::goes_through_door:
    {
      ...
      AddSubgoal(new Goal_NegotiateDoor_Z(m_pOwner, edge, m_Path.empty()));
    }
...
{ 
  ...
  Goal_Composite_Z<Raven_Bot_Z>::Render();
}
