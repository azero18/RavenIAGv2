#include "FleeFromTargetGoal_Evaluator_Z.h"
...
//-----------------------------------------------------------------------------
double FleeFromTargetGoal_Evaluator_Z::CalculateDesirability(Raven_Bot* pBot)
{
  ...
  else
  {
    ...
    double Desirability = Tweaker * (Raven_Feature::DistanceToItem(pBot, type_health))/
...
//----------------------------- SetGoal ---------------------------------------
//-----------------------------------------------------------------------------
void FleeFromTargetGoal_Evaluator_Z::SetGoal(Raven_Bot* pBot)
{
	if (pBot->GetTargetBot() == nullptr){
		pBot->GetBrain()->AddGoal_FleeFromPosition((pBot->Pos())); }
	else{pBot->GetBrain()->AddGoal_FleeFromPosition((pBot->GetTargetBot()->Pos()));
	}
}

//-------------------------- RenderInfo ---------------------------------------
//-----------------------------------------------------------------------------
void FleeFromTargetGoal_Evaluator_Z::RenderInfo(Vector2D Position, Raven_Bot* pBot)
{
